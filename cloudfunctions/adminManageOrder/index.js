...
async function listOrders({
  ...
  const list = (listRes.data || []).map((order) => ({
    ...
    challengeStatus: order.challengeStatus || null,
    challengeRefundStatus: order.challengeRefundStatus || null,
    erpStatus: order.erpStatus || "HOLD",
    challengeInfo: order.challengeInfo || null,
  }));
...
async function shipOrder(
  ...
  const hold = order.fulfillmentHold;
  if (hold && hold !== "NONE") {
    throw new Error(`该订单履约暂扣中（当前状态：${hold}），发货与ERP推单已被严格拦截，需等待挑战或免单退款结算完成`);
  }
...
async function confirmPickup({ orderId, remark }, adminInfo) {
  ...
  const hold = order.fulfillmentHold;
  if (hold && hold !== "NONE") {
    throw new Error(`该订单履约暂扣中（当前状态：${hold}），自提核销已拦截，需等待挑战或免单退款结算完成`);
  }
